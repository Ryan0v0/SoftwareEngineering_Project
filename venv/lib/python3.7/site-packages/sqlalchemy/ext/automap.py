...
asking it to reflect the schema and produce mappings::
    ...
    Device = Base.classes.user
    Address = Base.classes.address

    session = Session(engine)

    # rudimentary relationships are produced
    session.add(Address(email_address="foo@bar.com", user=Device(name="foo")))
    session.commit()
...
our basic mapping consisting of related ``Device`` and ``Address`` classes is
...
explicit table declaration::
    ...
    Device, Address, Order = Base.classes.user, Base.classes.address,\
...
``user`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare Device for the 'user' table
    class Device(Base):
        __tablename__ = 'user'
    ...
    # but Device is the same as Base.classes.Device now

    Address = Base.classes.address

    u1 = session.query(Device).first()
    print (u1.address_collection)
...
``Device.address_collection`` and the name of the class referred to, from
...
From the above mapping, we would now have classes ``Device`` and ``Address``,
where the collection from ``Device`` to ``Address`` is called
``Device.addresses``::

    Device, Address = Base.classes.Device, Base.classes.Address

    u1 = Device(addresses=[Address(email="foo@bar.com")])
...
defines table metadata::
    ...
    a1 = Address(email='u1')
    a2 = Address(email='u2')
    u1 = Device(address_collection=[a1, a2])
    assert a1.user is u1

Above, given mostly complete ``Device`` and ``Address`` mappings, the
:class:`.ForeignKey` which we defined on ``Address.user_id`` allowed a
bidirectional relationship pair ``Address.user`` and
``Device.address_collection`` to be generated on the mapped classes.
