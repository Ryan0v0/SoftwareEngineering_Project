...
mapping which relates a ``Device`` to a ``SavingsAccount``::
...
However, at the expression level, it's expected that the ``Device`` class will
be used in an appropriate context such that an appropriate join to
``SavingsAccount`` will be present::

    >>> print Session().query(Device, Device.balance).\
    ...     join(Device.accounts).filter(Device.balance > 5000)
...
would use an outer join::
    ...
    >>> print (Session().query(Device, Device.balance).outerjoin(Device.accounts).
    ...         filter(or_(Device.balance < 5000, Device.balance == None)))
    SELECT "user".id AS user_id, "user".name AS user_name,
    account.balance AS account_balance
...
a correlated SELECT::
    ...
    >>> print s.query(Device).filter(Device.balance > 400)
    SELECT "user".id AS user_id, "user".name AS user_name
    FROM "user"
