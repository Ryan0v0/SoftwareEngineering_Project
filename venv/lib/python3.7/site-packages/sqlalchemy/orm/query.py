...
class Query(object):
    ...
    def __init__(self, entities, session=None):
        ...
        E.g.::
            ...
            q = Query([Device, Address], session=some_session)

        The above is equivalent to::

            q = some_session.query(Device, Address)
    ...
    def get(self, ident):
        ...
        E.g.::
            ...
            my_user = session.query(Device).get(5)
    ...
    def from_self(self, *entities):
        ...
        into a SELECT of itself.  Given a query such as::
            ...
            q = session.query(Device).filter(Device.name.like('e%'))

        Given the :meth:`.Query.from_self` version::

            q = session.query(Device).filter(Device.name.like('e%')).from_self()
        ...
        joins against that row-limited set::
            ...
            q = session.query(Device).filter(Device.name.like('e%')).\
                limit(5).from_self().\
                join(Device.addresses).filter(Address.email.like('q%'))

        The above query joins to the ``Address`` entity but only against the
        first five results of the ``Device`` query:
        ...
        **automatic aliasing** to the entities inside the subquery, when
        ...
        refer to the ``Device`` entity without any additional aliasing applied
        ...
        The ORDER BY against ``Device.name`` is aliased to be in terms of the
        inner subquery:
        ...
        **Changing the Entities**
        ...
        columns are being queried.   In our example, we want ``Device.id``
        ...
        **Looking out for Inner / Outer Columns**
        ...
        columns.   Below illustrates a join of ``Address`` to ``Device``,
        then a subquery, and then we'd like :func:`.contains_eager` to access
        the ``Device`` columns::

            q = session.query(Address).join(Address.user).\
                filter(Device.name.like('e%'))

            q = q.add_entity(Device).from_self().\
                options(contains_eager(Address.user))

        We use :meth:`.Query.add_entity` above **before** we call
        :meth:`.Query.from_self` so that the ``Device`` columns are present
        ...
        If we didn't call ``add_entity(Device)``, but still asked
        :func:`.contains_eager` to load the ``Device`` entity, it would be
    ...
    def with_entities(self, *entities):
        ...
        e.g.::
            ...
            q = session.query(Device).\
                        join(Device.address).\
                        filter(Device.name.like('%ed%')).\
                        order_by(Address.email)

            # given *only* Device.id==5, Address.email, and 'q', what
            # would the *next* Device in the result be ?
    ...
    def with_for_update(self, read=False, nowait=False, of=None,
        ...
        E.g.::
            ...
            q = sess.query(Device).with_for_update(nowait=True, of=Device)
    ...
    def join(self, *props, **kwargs):
        ...
        **Simple Relationship Joins**
        ...
        Consider a mapping between two classes ``Device`` and ``Address``,
        with a relationship ``Device.addresses`` representing a collection
        of ``Address`` objects associated with each ``Device``.   The most
        common usage of :meth:`~.Query.join` is to create a JOIN along this
        relationship, using the ``Device.addresses`` attribute as an indicator
        for how this should occur::

            q = session.query(Device).join(Device.addresses)

        Where above, the call to :meth:`~.Query.join` along ``Device.addresses``
        will result in SQL equivalent to::

            SELECT user.* FROM user JOIN address ON user.id = address.user_id

        In the above example we refer to ``Device.addresses`` as passed to
        ...
        only from ``Device`` and nothing else), the relationship can also be
        specified by its string name::

            q = session.query(Device).join("addresses")
        ...
        where a join across four related entities is constructed::
            ...
            q = session.query(Device).join("orders", "items", "keywords")
        ...
        **Joins to a Target Entity or Selectable**
        ...
        two entities::
            ...
            q = session.query(Device).join(Address)
        ...
        be any mapped entity or selectable, such as a :class:`.Table`::
            ...
            q = session.query(Device).join(addresses_table)
        ...
        **Joins to a Target with an ON Clause**
        ...
        entity in some way as well as an applicable source entity::
            ...
            q = session.query(Device).join(Address, Device.id==Address.user_id)

        .. versionchanged:: 0.7
            In SQLAlchemy 0.6 and earlier, the two argument form of
            :meth:`~.Query.join` requires the usage of a tuple:
            ``query(Device).join((Address, Device.id==Address.user_id))``\ .
        ...
        **Advanced Join Targeting and Adaption**
        ...
        or two-argument forms::
            ...
            q = session.query(Device).\
                        join(addresses_q, addresses_q.c.user_id==Device.id)

        :meth:`~.Query.join` also features the ability to *adapt* a
        :meth:`~sqlalchemy.orm.relationship` -driven ON clause to the target
        selectable. Below we construct a JOIN from ``Device`` to a subquery
        against ``Address``, allowing the relationship denoted by
        ``Device.addresses`` to *adapt* itself to the altered target::

            address_subq = session.query(Address).\
                            filter(Address.email_address == 'ed@foo.com').\
                            subquery()

            q = session.query(Device).join(address_subq, Device.addresses)
        ...
        clause at any time::
            ...
            q = session.query(Device).\
                    join(address_subq, Device.id==address_subq.c.user_id)
        ...
        **Controlling what to Join From**
        ...
        make usage of ``Device.addresses`` as our ON clause by instructing
        the :class:`.Query` to select first from the ``Device``
        entity::

            q = session.query(Address).select_from(Device).\
                            join(Device.addresses).\
                            filter(Device.name == 'ed')
    ...
    def select_entity_from(self, from_obj):
        ...
        Given a case for :func:`.aliased` such as selecting ``Device``
        objects from a SELECT statement::

            select_stmt = select([Device]).where(Device.id == 7)
            user_alias = aliased(Device, select_stmt)
        ...
        used at the start of the query to adapt the existing ``Device`` entity::

            q = session.query(Device).\
                select_entity_from(select_stmt).\
                filter(Device.name == 'ed')

        Above, the generated SQL will show that the ``Device`` entity is
        ...
        :meth:`.TextClause.columns` method::
            ...
            q = session.query(Device).select_entity_from(text_stmt)
        ...
        we can use name matching to select from it::
            ...
            user_view_alias = aliased(
                Device, user_view, adapt_on_names=True)
            q = session.query(Device).\
    ...
    def slice(self, start, stop):
        ...
        For example, ::
            ...
            session.query(Device).order_by(Device.id).slice(1, 3)
    ...
    def column_descriptions(self):
        ...
        Format is a list of dictionaries::
            ...
            user_alias = aliased(Device, name='user2')
            q = sess.query(Device, Device.id, user_alias)
    ...
    def instances(self, cursor, __context=None):
        ...
        e.g.::
            ...
            for u in session.query(Device).instances(result):
                print u
        """
        context = __context
        if context is None:
    ...
    def exists(self):
        ...
        e.g.::
            ...
            q = session.query(Device).filter(Device.name == 'fred')
        ...
        The EXISTS construct is usually used in the WHERE clause::
            ...
            session.query(Device.id).filter(q.exists()).scalar()
    ...
    def count(self):
        ...
        with :meth:`~.Session.query`, i.e.::
            ...
            session.query(func.count(distinct(Device.name)))
    ...
    def delete(self, synchronize_session='evaluate'):
        ...
        E.g.::
            ...
            sess.query(Device).filter(Device.age == 25).\
    ...
    def update(self, values, synchronize_session='evaluate', update_args=None):
        ...
        E.g.::
            ...
            sess.query(Device).filter(Device.age == 25).\
                update({Device.age: Device.age - 10}, synchronize_session=False)

            sess.query(Device).filter(Device.age == 25).\
                update({"age": Device.age - 10}, synchronize_session='evaluate')
    ...
    def __init__(self, alias):
        ...
        statement that aliases the parent table.  E.g.::
            ...
            query = session.query(Device).options(
                                    ...
                                    contains_eager(Device.addresses))
