...
class Load(Generative, MapperOption):
    ...
    method-chaining approach.  Supposing a class hierarchy such as ``Device``,
    ``Device.addresses -> Address``, ``Device.orders -> Order`` and
    ``Order.items -> Item``, we can specify a variety of loader options along
    each element in the "path"::

        session.query(Device).options(
...
def contains_eager(loadopt, attr, alias=None):
    ...
    ``Device`` entity, and the returned ``Order`` objects would have the
...
def load_only(loadopt, *attrs):
    ...
    Example - given a class ``Device``, load only the ``name`` and ``fullname``
    attributes::

        session.query(Device).options(load_only("name", "fullname"))

    Example - given a relationship ``Device.addresses -> Address``, specify
    subquery loading for the ``Device.addresses`` collection, but on each
    ...
    specifically referred to using the :class:`.Load` constructor::
        ...
        session.query(Device, Address).join(Device.addresses).options(
                    Load(Device).load_only("name", "fullname"),
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    examples::
        ...
        # joined-load the "orders" collection on "Device"
        query(Device).options(joinedload(Device.orders))
...
def subqueryload(loadopt, attr):
    ...
    examples::
        ...
        # subquery-load the "orders" collection on "Device"
        query(Device).options(subqueryload(Device.orders))
