...
class ColumnElement(operators.ColumnOperators, ClauseElement):
    ...
    functions with regards to SQL expressions are as follows:
        ...
        * any special object value, typically ORM-level constructs, which
          ...
          attributes on ORM-mapped classes; a ``Device`` class which contains a
          mapped attribute named ``.name`` will have a method
          ``Device.name.__clause_element__()`` which when invoked returns the
...
class TextClause(Executable, ClauseElement):
    ...
    def columns(self, selectable, *cols, **types):
        ...
        name or origin of the column in the textual SQL doesn't matter::
            ...
            query = session.query(Device).from_statement(stmt).options(
